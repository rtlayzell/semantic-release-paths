...
import { filterCommitsByPaths } from "./paths/index.js";
...
export default async ({
  ...
  if (paths && paths.length > 0) {
    commits = await filterCommitsByPaths(commits, paths, { cwd, env });
    logger.log(`Found ${commits.length} commits matching path filters`);
  }
