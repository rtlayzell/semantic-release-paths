...
/**
 * Get changed files for a given commit hash.
 * @param {String} commitHash The commit hash to retrieve the changed files for.
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @return {Array<String>} List of changed files.
 * @throws {Error} If the `git` command fails.
 */
export async function getChangedFiles(commitHash, execaOptions) {
  return (
    await execa("git", ["diff-tree", "--no-commit-id", "--root", "--name-only", "-r", commitHash], execaOptions)
  ).stdout
    .split("\n")
    .filter(Boolean);
}
