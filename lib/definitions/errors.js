...
export function EINVALIDPATH({ pattern }) {
  return {
    message: "A path pattern is invalid in the `paths` configuration.",
    details: `Each path pattern in the [paths configuration](${linkify(
      "docs/usage/configuration.md#paths"
    )}) must be a non-empty string.

Your configuration for the problematic path pattern is \`${stringify(pattern)}\`.`,
  };
}

export function EABSOLUTEPATH({ pattern }) {
  return {
    message: "A path pattern is invalid in the `paths` configuration.",
    details: `Each path pattern in the [paths configuration](${linkify(
      "docs/usage/configuration.md#paths"
    )}) must be a repository-relative glob, not an absolute path.
